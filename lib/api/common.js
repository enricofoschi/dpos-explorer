module.exports = function (app, api) {
		...
		delegates.getSearch(
			...
			(body) => {
				if (body.success === true) {
					return success({ success: true, results: body.results });
				}
	...
	const searchAccount = function (id, error, success) {
		...
		accounts.getAccount(
			...
			(body) => {
				if (body.success === true) {
					return success({ success: true, results: [{ type: 'address', id }] });
				}
				return error({ success: false, error: null, found: false });
			});
	};
	...
	const searchPublicKey = function (id, error, success) {
		...
		accounts.getAccount(
			...
			(body) => {
				if (body.success === true) {
					return success({ success: true, results: [{ type: 'address', id: body.address }] });
				}
				return error({ success: false, error: null, found: false });
			});
	};
	...
	const searchTransaction = function (id, error, success) {
		...
		transactions.getTransaction(
			...
			(body) => {
				if (body.success === true) {
					return success({ success: true, results: [{ type: 'tx', id: body.transaction.id }] });
				}
	...
	const searchBlock = function (id, error, success) {
		...
		blocks.getBlock(
			...
			(body) => {
				if (body.success === true) {
					return success({ success: true, results: [{ type: 'block', id: body.block.id }] });
				}
				return error({ success: false, error: body.error });
			});
	};
