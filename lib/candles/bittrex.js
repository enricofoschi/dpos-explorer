...
function BittrexCandles(...rest) {
	...
	// this.url = 'https://bittrex.com/Api/v2.0/pub/market/GetTicks?marketName=BTC-LSK&tickInterval=thirtyMin';
	...
	this.parseCandles = function (results, cb) {
		...
		let sum = _.map(results, period => ({
			timestamp: self.parseDate(period[0][mapping.date]).unix(),
			date: self.parseDate(period[0][mapping.date]).toDate(),
			high: _.max(period, t => parseFloat(t[mapping.high]))[mapping.high],
			low: _.min(period, t => parseFloat(t[mapping.low]))[mapping.low],
			open: _.first(period)[mapping.open],
			close: _.last(period)[mapping.close],
